...
public class Liquid_Output extends Block {
   public Liquid_Output(String name) {
      super(name);
      this.update = true;
      this.solid = true;
      this.health = 500;
   ...
   public class Liquid_OutputBuild extends Building {
      ...
      public void updateTile() {
         if (this.selectedLiquid != null && this.team.core() != null) {
            ...
            if (this.core_X.interactable(this.team) && this.core_X.block.hasLiquids && this.core_X.liquids.get(this.selectedLiquid) >= 1.0F) {
               this.dumpingTo = this.core_X;
               if (this.putLiquid(this.selectedLiquid)) {
                  this.core_X.liquids.remove(this.selectedLiquid, 1.0F);
               }
            }
         }
      ...
      public void draw() {
         ...
         if (this.selectedLiquid != null) {
            ...
            Draw.rect("coreunloader-mod-Liquid-Output-center", this.x, this.y);
            Draw.color();
         }
      }
      ...
      public boolean canDump(Building to, Liquid liquid) {
      ...
      public boolean putLiquid(Liquid liquid) {
         int dump = this.cdump;

         for(int i = 0; i < this.proximity.size; ++i) {
            this.incrementDump(this.proximity.size);
            Building other = (Building)this.proximity.get((i + dump) % this.proximity.size);
            if (other.team == this.team && other.acceptLiquid(this, liquid) && this.canDump(other, liquid)) {
               other.handleLiquid(this, liquid, 1.0F);
               return true;
            }
         }
